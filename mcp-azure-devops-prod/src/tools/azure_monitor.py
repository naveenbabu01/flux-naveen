"""
tools/azure_monitor.py — Production Azure Monitor tools using DefaultAzureCredential.
"""
...
class AzureMonitorTools:
    ...
    async def get_alerts(self, resource_group: str, severity: str = None) -> dict:
        ...
        try:
            alerts = self.monitor_client.alerts.get_all(resource_group_name=resource_group)
            result = []
            ...
            return {"resource_group": resource_group, "total_alerts": len(result), "alerts": result}
        except Exception as e:
            return {"error": str(e), "resource_group": resource_group}

    async def get_app_insights_errors(self, app_name: str, hours: int = 24, limit: int = 50) -> dict:
        """Query Application Insights for exceptions in the last N hours."""
        if not self.cfg.app_insights_app_id or not self.cfg.app_insights_api_key:
            return {"error": "APP_INSIGHTS_APP_ID and APP_INSIGHTS_API_KEY required"}
...
| take {limit}
"""
        url = f"https://api.applicationinsights.io/v1/apps/{self.cfg.app_insights_app_id}/query"
        ...
        try:
            ...
            return {"app_name": app_name, "period_hours": hours, "errors": errors}
        except Exception as e:
